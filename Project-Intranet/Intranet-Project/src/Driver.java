...
public class Driver {
	...
	public static void deserializeObjects() throws IOException, ClassNotFoundException{
		...
		if(new File("executors.ser").exists()) Database.desExecutors();
	...
	public static void main(String[] args) throws CloneNotSupportedException, ClassNotFoundException {
		...
		try {
			...
			/*clear();
			Admin a = new Admin("Admin", "Adminov", "16AD00001", "admin");
			...
			Executor e = new Executor("Executor", "Executorov", "16ED00001", "111");
			Database.users.add(e);
			Database.executors.add(e);
			*/
								...
								if(passwordCheck) {
									...
									if(user instanceof Admin) {
													...
													if(choice.equals("1")) {
														adding : while(true) {
															...
															if(choice.equals("1")) {
																...
																((Admin) user).addUser(new Executor(inputName, inputSurname, inputId, inputPassword));
																Database.executors.add(new Executor(inputName, inputSurname, inputId, inputPassword));
																
																System.out.println(("The Executor: \n" + new Executor(inputName, inputSurname, inputId, inputPassword).toString() + "\nwas added sucessfully!"));
															}
										...
										while(true) {
											...
											choice = in.readLine();
											if(choice.equals("1")) {
												System.out.println(((Executor) user).getOrder().toString());			
											}
											else if(choice.equals("2")){
												System.out.println("Enter id of the order");
												choice  = in.readLine();
												((Executor) user).doneOrder(Database.orders.get(Integer.parseInt(choice)-1));
											}
											else if(choice.equals("3")){
												System.out.println(((Executor) user).viewDoneOrders());
											}
											else {
												break;
											}
										...
										while(true) {
											...
											System.out.println("6) to register to course"); 
											...
											if(choice.equals("1")){
												...
												*/
												...
												//CurrentTeacher.setStudenttoCourse(currentCourse, ((Student) user));
											}
...
//																					CurTeacher.setCourse(k, current);
												System.out.println(CurTeacher.getCourses());
										...
										while(true) {
											...
											if(choice.equals("1")){
												...
												/* for(int i = 0; i< ((Teacher) user).courses.size(); i++) {
												    	System.out.println(((Teacher) user).students.get(i).toString());
...
//		System.out.println();
		...
		*/
		...
		//System.out.println(CurrentTeacher.students.get(currentCourse).toString());
